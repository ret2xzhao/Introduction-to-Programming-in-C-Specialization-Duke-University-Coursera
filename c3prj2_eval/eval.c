...
int is_n_length_straight_at(deck_t * hand, size_t index, suit_t fs, int n) {
  ...
  unsigned next_straight_value = 5;
  for (int i = index + 1; i < hand->n_cards; i++) {
    if (hand->cards[i]->value == next_straight_value && 
...
int is_reg_straight_(deck_t * hand, size_t index, suit_t fs, int n) {  
  if (fs != NUM_SUITS && fs != hand->cards[index]->suit) {
    return 0;
  }
  int straight_cards = 1;
  int next_straight_value = (hand->cards[index]->value)-1;
  for (int i = index + 1; i < hand->n_cards; i++) {
    if ((hand->cards[i]->value == next_straight_value) && 
        (fs == NUM_SUITS || fs == hand->cards[i]->suit)) {
      straight_cards++;
...
int check_ace_low(deck_t * hand, size_t index, suit_t fs) {
  if (hand->cards[index]->value == 14 && (fs == NUM_SUITS || fs == (hand->cards[index]->suit))) {
    return 1;
  }
  return 0;
}
...
int is_straight_at(deck_t * hand, size_t index, suit_t fs) {
  if (check_ace_low(hand, index, fs) && is_reg_straight_(hand, index, fs, 4)) {/* check if is AceLowStraight*/
    return -1;
  }
  else if (is_reg_straight_(hand, index, fs, 5)) {/* check if is RegularStraight*/
    return 1;
  }
  return 0;
}
